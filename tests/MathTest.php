...
class MathTest extends TestCase
{
    ...
    public function testNonNumericComparisonWithoutHandler() : void
    {
        ...
        $this->assertEquals('SC', $calculator->execute('+rating'));
    }
    ...
    {
        $calculator = new MathExecutor();
        $calculator->setNonNumericHandler(static fn($value, $operator) => 'SC' === $value ? 10 : 0);
        $calculator->setVar('rating', 'SC');
        $calculator->setVar('other', 'DC');
    ...
    public function testNonNumericHandlerException() : void
    {
        ...
        $this->expectException(MathExecutorException::class);
        $this->expectExceptionMessage('Value (SC) is not a number, required by operator (/)');
    ...
    public function testNonNumericHandlerDoesNotAffectStringOperators() : void
    {
        ...
        $this->assertEquals(true, $calculator->execute("rating == 'SC'"));
        $this->assertEquals(false, $calculator->execute("rating != 'SC'"));
        $this->assertEquals(true, $calculator->execute("rating != 'DC'"));
        $this->assertEquals(true, $calculator->execute('rating && 1'));
